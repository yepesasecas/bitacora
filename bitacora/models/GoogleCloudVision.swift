...
class GoogleCloudVision: NSObject {
    ...
    func objectsIn(image: UIImage, compressionQuality: CGFloat, closure: @escaping (Any?, Any?) -> Void) {
        ...
        let task = URLSession.shared.dataTask(with: urlRequest) {(data, response, error) in
            ...
            guard let resJson = try? JSONSerialization.jsonObject(with: data, options: []) as AnyObject,
                  ...
                  let labelAnnotationsJson = responsesJson[0]["labelAnnotations"] as? [AnyObject] else {
                      closure(nil, "unable to decode Data to JSON Object")
                      return
            }
            
            let annotations = labelAnnotationsJson.map { GCVLabelAnnotation(dictionary: $0)}
            closure(annotations, nil)
        }
